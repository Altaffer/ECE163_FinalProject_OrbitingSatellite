...
orbitalPitch = [0 if (i>100) else 0 for i in range(n_steps)]
...
orbit=[[0],[0],[-(400000+VPC.radius_e)]]
for i in range(n_steps):
    Re2o, Ro2e = of.orbitalFrameR(orbit, vs)
    ...
    oYaw, oPitch, oRoll = Rotations.dcm2Euler(Re2o)
    orbitalYaw[i] = math.degrees(oYaw)
    orbitalPitch[i] = math.degrees(oPitch)
    orbitalRoll[i] = math.degrees(oRoll)
